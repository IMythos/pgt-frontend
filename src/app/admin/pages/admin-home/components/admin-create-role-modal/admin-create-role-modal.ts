...
@Component({
  selector: 'app-admin-create-role-modal',
  imports: [CommonModule, FormsModule, Modal, ModalHeader, ModalFooter],
  templateUrl: './admin-create-role-modal.html',
})
export class AdminCreateRoleModal {
  ...
  save(): void {
    ...
    this.adminApi
      ...
      .subscribe({
        ...
        error: (error) => {
          const msg = error.error?.message ?? 'No se pudo crear el rol.';
