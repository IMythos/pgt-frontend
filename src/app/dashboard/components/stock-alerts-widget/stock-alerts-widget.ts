...
@Component({
  ...
  template: `
    <div class="flex flex-col items-start p-6 h-[400px] bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-[#1F1F1F] shadow-sm rounded-xl transition-all duration-300 relative overflow-hidden">
      ...
      <div class="flex flex-col gap-2 w-full flex-1 overflow-y-auto pr-2">
        @if (data().length === 0) {
          <div class="flex flex-col items-center justify-center h-full gap-3 text-center">
            <svg class="w-12 h-12 text-[#34A853] opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <p class="font-['Inter'] font-medium text-sm text-[#4C616C] dark:text-[#8A9BA8]">No hay alertas de stock</p>
            <p class="font-['Inter'] text-xs text-[#8A9BA8] dark:text-[#6B7D8A]">Todos los repuestos están dentro del mínimo operativo</p>
          </div>
        } @else {
      ...
      <a (click)="showModal.set(true)" class="mt-4 font-['Inter'] font-bold text-sm text-[#81000A] dark:text-[#EF4444] hover:underline transition-colors w-full text-center py-2 cursor-pointer">
        Ver todas las alertas
      </a>
    </div>

    @if (showModal()) {
      <app-stock-alerts-modal
        [alerts]="data()"
